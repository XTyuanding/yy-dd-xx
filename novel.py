...
class download_novel(object):
    ...
    def __init__(self):
        ...
        # 沙海
        # self.target_url = "http://www.biqukan.com/33_33964/"
        # 星辰变
        self.target_url = "http://www.xbiquge.la/14/14831/"
    ...
    def get_download_url(self):
        ...
        # 切片，除掉前面不需要的部分
        self.nums = len(a_list[0:])
        # 开始迭代页面
        for each in a_list[0:]:
...
for i in range(novel.nums):
    novel.write_content(
        novel.title[i], '不朽剑神.txt', novel.get_content(novel.target_urls[i]))
